...
SERVER_URL=${PRIVATECLOUD_SERVER_URL:-http://SERVER_IP:8765}
STAMP=$(date +%Y%m%d-%H%M%S)
WORKDIR=${WORKDIR:-privatecloud/tmp/ksdk-catalog-$STAMP}
...
echo "Pulling current cc.db and ksdk.asset.db"
scp -F /dev/null "$KINDLE_HOST:/var/local/cc.db" "$WORKDIR/cc.source.db"
scp -F /dev/null "$KINDLE_HOST:/var/local/ksdk.asset.db" "$WORKDIR/ksdk.asset.source.db"

echo "Building modified cc.db"
python3 privatecloud/catalog_import.py \
    --db "$WORKDIR/cc.source.db" \
    --manifest "$WORKDIR/manifest.tsv" \
    --output "$WORKDIR/cc.private.db"

echo "Building modified ksdk.asset.db"
python3 privatecloud/ksdk_asset_import.py \
    --db "$WORKDIR/ksdk.asset.source.db" \
    --cc-db "$WORKDIR/cc.source.db" \
    --manifest "$WORKDIR/manifest.tsv" \
    --output "$WORKDIR/ksdk.asset.private.db"

echo "Pushing modified databases and installing with backup $BACKUP_DIR"
ssh -F /dev/null "$KINDLE_HOST" "mkdir -p '$BACKUP_DIR'"
scp -F /dev/null "$WORKDIR/cc.private.db" "$KINDLE_HOST:$BACKUP_DIR/cc.private.db"
scp -F /dev/null "$WORKDIR/ksdk.asset.private.db" "$KINDLE_HOST:$BACKUP_DIR/ksdk.asset.private.db"
ssh -F /dev/null "$KINDLE_HOST" "set -e
cp /var/local/cc.db '$BACKUP_DIR/cc.before-private.db'
cp /var/local/ksdk.asset.db '$BACKUP_DIR/ksdk.asset.before-private.db'
...
cat '$BACKUP_DIR/ksdk.asset.private.db' > /var/local/ksdk.asset.db
...
echo '$BACKUP_DIR' > /mnt/us/privatecloud/last-ksdk-catalog-test-backup
"

echo "Installed private cc+ksdk catalog test rows."
